/**
 * jquery.flips.js
 * Author: Alexander Dümont / newtork
...
(function( window, undefined ) {
	...
	$.Flips.defaults 	= {
		...
		heightAbs			: -30,
		heightRel			: 1,
		widthAbs			: function() { return -parseInt($("#flipout").css("margin-right")||0)  - parseInt($("#flipout").css("margin-left")||0); },
		widthRel			: 1,
		rotationDeg			: -7,
	};
	...
	$.Flips.prototype 	= {
		_init 				: function( options ) {
			...
			this.HashPrefixLength = this.HashPrefix.length;
		},
		...
		_getWinSize			: function() {
			...
			this.windowProp = {
				width	: $win.width() * this.options.widthRel + this.options.widthAbs(),
				height	: $win.height() * this.options.heightRel + this.options.heightAbs,
				top		: offs.top,
				left	: offs.left,
				rotationDegSin	: Math.sin(this.options.rotationDeg/180 * Math.PI),
				rotationDegCos	:  Math.cos(this.options.rotationDeg/180 * Math.PI)
			};
		},
		_goto				: function() {
		...
		_getState			: function() {
			...
			this.state = h && h.substr(0, this.HashPrefixLength)==this.HashPrefix && h.substr(this.HashPrefixLength) || undefined;
		},
		...
		_layout				: function() {
			...
			for( var i = 0; i <= this.pagesCount - 2; ++i ) {
				...
				var appendableHTML = $.templates('#pageTmpl').render( pageData )
				$(appendableHTML).appendTo( this.$el );
			}
		...
		_initTouchSwipe		: function() {
			...
			var _rotMatrix = function(x,y) {
				if(_self.centerX===null || _self.centerY===null) {
					// left
					{
						var left = _self.$el.context.offsetLeft + _self.$el.context.offsetWidth/2;
						if(_self.options.rotationDeg)
							left += _self.$el.context.parentNode.offsetLeft;
						_self.centerX = left;
					}
					// top
					{
						var top = _self.$el.context.offsetTop + _self.$el.context.offsetHeight/2;
						if(_self.options.rotationDeg)
							top += _self.$el.context.parentNode.offsetTop;
						_self.centerY = top;
					}
				}
				return {
					x : _self.centerX + _self.windowProp.rotationDegCos*(x-_self.centerX) + _self.windowProp.rotationDegSin*(y-_self.centerY),
					y : _self.centerY - _self.windowProp.rotationDegSin*(x-_self.centerX) + _self.windowProp.rotationDegCos*(y-_self.centerY)
				};
			}
			...
			this.$el.swipe( {
				...
				swipeStatus			: function( event, phase, direction, distance, duration, fingerCount, fingerData) {
					// event \element [ start, move, end ]
					// phase \element [ null, left, right ]
					// distance \element N
					// duration \element N_0^+
					
					{
						// handle a bug, which freezes the swipe: start,cancel,start,cancel,...
						if (phase=="cancel" && _self._isAnimating() && _self.previousPhase=="start") {
							_self.$el.swipe("disable");
							_self.$el.swipe("enable");
							console.log("Freeze, restarting swipe.");
							return
						}
						_self.previousPhase = phase;
					}

					var endX, endY;
					{
						// claculate current "end" coordinates
						var currentX = event.clientX
										|| event.touches && event.touches.length && event.touches[0].clientX
										|| fingerData && fingerData[0].start.x;
						var currentY = event.clientY
										|| event.touches && event.touches.length && event.touches[0].clientY
										|| fingerData && fingerData[0].start.y;
						with(_rotMatrix(currentX, currentY)) {
							endX = x;
							endY = y;
						}
					}
					
					// copy start coordinates
					if (phase=="start") {
						_self.startX = endX;
						_self.startY = endY;
					}
					...
					// only take left/right
					// TODO consider rotation=90° + top/down -> left/right
					direction = ["left", "right"].indexOf(direction)>=0 ? direction : "left";
					...
					if( !_self._isAnimating() ) {
						_self.flipSide = ( _self.startX < _self.centerX ) ?  'l2r' : 'r2l';
					}
					...
					sym	= _self.centerX - _self.startX;
					var symMargin = 0.9 * ( _self.windowProp.width / 2 );
					if( Math.abs( _self.startX - sym ) < symMargin ) {
					...
					// |          |          |
					// -----------------------
					if( endX > _self.startX && _self.flipSide === 'r2l' ) {
						console.log("r2l stop 0");
						angle		= 0;
						oob 		= true;
						noflip		= true;
					}
					...
					// |          |          |
					// -----------------------
					else if( endX < _self.startX && _self.flipSide === 'l2r' ) {
						console.log("l2r stop 180");
						angle		= 180;
						oob 		= true;
						noflip		= true;
					}
					...
					// -----------------------
						...
						( _self.flipSide === 'r2l' ) ?
							( s = _self.startX, e = sym, val = _self.startX - distance ) : 
							( s = sym, e = _self.startX , val = _self.startX + distance );
		...
		_setFlippingPage	: function() {
			var _self = this;
			this.$flippingPage = this.flipSide === 'l2r'
									? this.$flipPages.eq( this.currentPage - 1 )
									: this.$flipPages.eq( this.currentPage );
		...
		_loadEvents			: function() {
			...
			$( window ).on( 'resize.flips', function( event ) {
				...
				_self.centerX = _self.centerY = null;
			} );
			
			$( window ).on( 'statechange.flips hashchange', function( event ) {
